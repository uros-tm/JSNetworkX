...
"default"], {
  ...
  /*istanbul ignore next*/
  testName() {
    var G = new this.Graph(null, {
      name: ''
    });
    assert.equal(G.name, '');
    G = new this.Graph(null, {
      name: 'test'
    });
  ...
  testCopy() {
    ...
    var H = G.copy(); //shared.is_deepcopy(H, G);
  ...
  testNodeAttr() {
    ...
    assert.deepEqual(G.nodes(true), [[0, {}], [1, {
      'foo': 'bar'
    }], [2, {}]]);
  ...
  testNodeAttr2() {
    ...
    assert.deepEqual(G.nodes(true), [[0, {}], [1, {}], [2, {}], [3, {
      'foo': 'bar'
    }]]);
  },
  ...
  testEdgeAttr2() {
    ...
    assert.deepEqual(G.edges(true).sort(), [[1, 2, {
      'foo': 'foo'
    }], [3, 4, {
      'foo': 'foo'
    }]]);
  ...
  testSubgraph() {
    ...
    var H = G.subgraph([0, 1, 2, 5]); // assert.equal(H.name, 'Subgraph of ('+G.name+')')
    ...
    _shared[
    /*istanbul ignore next*/
    "default"].graphsEqual(H, G);

    /*istanbul ignore next*/
    _shared[
    /*istanbul ignore next*/
    "default"].sameAttrdict(H, G);

    /*istanbul ignore next*/
    _shared[
    /*istanbul ignore next*/
    "default"].shallowCopyAttrdict(H, G);
  ...
  testSelfloopsAttr() {
    ...
    assert.deepEqual(G.selfloopEdges(true), [[0, 0, {}], [1, 1, {
      weight: 2
    }]]);
  }
});
